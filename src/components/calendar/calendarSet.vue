<template>
    <div class="calendar calendarset">
        ...
        <table cellpadding="5">
            ...
            <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
                <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" >
                    <div class="bomdates" @click="select(k1,k2,$event)">
                    ...
                    <template v-for="(item,k) in setdates">
                        <template v-if="year == item.year && (month + 1) == item.month && child.day == item.day">
                            <div v-if="item.passFlag == 0 && !child.disabled" class="workday" @click="updateDuty(item,k1,k2,k,$event)">
                                <strong class="logotext">作</strong>
                                <p>{{item.remark}}</p>
                            </div>
                            <div v-else-if="item.passFlag == 1 && !child.disabled" class="outworkday" @click="updateDuty(item,k1,k2,k,$event)">
                                <strong class="logotext">休</strong>
                                <p>{{item.remark}}</p>
                            </div>
                        </template>
                    </template>
...
<script>
import { changeDouble,getYearFormat } from '@/utils/index'
        ...
        updateDuty (item, k1, k2, k,e) {
          if (e != undefined) e.stopPropagation()
            ...
            if (this.range) {
                ...
                this.days[k1][k2].selected = !this.days[k1][k2].selected
                this.$emit('update',this.multiDays)
            } else {
                // 取消上次选中
                if (this.today.length > 0) {
                    this.days.forEach(v=>{
