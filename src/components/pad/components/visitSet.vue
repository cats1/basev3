...
export default {
    ...
    data() {
        return {
            ...
            extendPhoneNotRequired: process.env.extendPhoneNotRequired || false
        }
    },
    created() {
        ...
        if (this.extendPhoneNotRequired) {
            this.fieldname_arr = ['name', 'visitType', 'empid']
            this.addition_arr = ['phone','email', 'vcompany', 'gatein', 'gateout', 'guardin', 'guardout']
        }
    ...
    methods: {
        ...
        saveEdit(item, index) {
            if (item.displayName != '') {
                ...
                this.g_config_arr[index].isDisplay = item.isDisplay
        ...
        getExtendVisitor() {
            ...
            this.$store.dispatch('GetExtendVisitor', nform).then(res => {
                ...
                if (status === 0) {
                    ...
                    if (this.extendPhoneNotRequired) {
                        local_fieldname_arr = ['name', 'visitType', 'empid']
                        local_add_arr = ['phone','email', 'vcompany', 'gatein', 'gateout', 'guardin', 'guardout']
                    }
                    if (this.defaultVsetIsDisplayShow) {
                ...
                if (!g_config_arr[i].addtion) {
                    ...
                    if (this.defaultVsetIsDisplayShow||this.defaultVsetIsDisplayPointShow) {
                        tobj = {
                            ...
                            required: g_config_arr[i].required,
                            placeholder: g_config_arr[i].placeholder,
                            isDisplay: g_config_arr[i].isDisplay ? g_config_arr[i].isDisplay : 0
                        }
                    }
